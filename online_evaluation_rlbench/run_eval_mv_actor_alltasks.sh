...
CLIP_RN50_LOCAL_PATH=${CLIP_RN50_LOCAL_PATH:?Set CLIP_RN50_LOCAL_PATH to a local OpenAI CLIP RN50 .pt checkpoint.}
PI3_ROOT=${PI3_ROOT:?Set PI3_ROOT to the local Pi3 code directory.}
PI3X_CKPT=${PI3X_CKPT:?Set PI3X_CKPT to the local Pi3X checkpoint path.}
...
export CKPT DATA_DIR CLIP_HF_LOCAL_PATH CLIP_RN50_LOCAL_PATH PI3_ROOT PI3X_CKPT COPPELIASIM_ROOT RLBENCH_ROOT PYREP_ROOT CUDA_VISIBLE_DEVICES
...
for task_name in "${TASKS[@]}"; do
  ...
  DISPLAY_NUM="$DISPLAY_NUM" XVFB_LOG_FILE="$task_dir/xvfb.log" \
    bash online_evaluation_rlbench/with_xvfb.sh "$PY" online_evaluation_rlbench/evaluate_policy_inprocess_pi3x.py \
      --checkpoint "$CKPT" \
      --task "$task_name" \
