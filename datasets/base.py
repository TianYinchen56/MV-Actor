...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        # Sanity check
        len_ = _zarr_array_length(self.annos['action'])
        for key in self.annos:
            assert _zarr_array_length(self.annos[key]) == len_, f'length mismatch in {key}'
        print(f"Found {_zarr_array_length(self.annos['action'])} samples")
    ...
    def __len__(self):
        return self.copies * (_zarr_array_length(self.annos['action']) // self.chunk_size)
