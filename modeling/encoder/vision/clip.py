...
def _resolve_local_clip_rn50_path():
    explicit_path = os.environ.get("CLIP_RN50_LOCAL_PATH")
    if not explicit_path:
        raise FileNotFoundError(
            "CLIP_RN50_LOCAL_PATH is required. Point it to a local OpenAI CLIP RN50 .pt checkpoint."
        )
    ...
    if path.is_file():
        ...
        candidates.extend([path / "RN50.pt", path / "rn50.pt"])
        candidates.extend(sorted(path.glob("*RN50*.pt")))
        candidates.extend(sorted(path.glob("*rn50*.pt")))
    for candidate in candidates:
        if candidate.is_file():
            return str(candidate)
    raise FileNotFoundError(
        f"CLIP_RN50_LOCAL_PATH does not point to a valid local CLIP RN50 checkpoint: {explicit_path}"
    )


def load_clip():
    model_path = _resolve_local_clip_rn50_path()
    clip_model, clip_transforms = clip.load(model_path, device="cpu", jit=False)
