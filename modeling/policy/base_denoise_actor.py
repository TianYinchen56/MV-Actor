...
class DenoiseActor(nn.Module):
    ...
    def encode_inputs(self, rgb3d, rgb2d, pcd, instruction, proprio, extra_inputs=None):
    ...
    def policy_forward_pass(self, trajectory, timestep, fixed_inputs):
        ...
        return self.prediction_head(
            ...
            rgb3d_feats_pos=rgb3d_feats_pos,
            fps_scene_feats_pos=fps_scene_feats_pos,
            fps_scene_pos_pos=fps_scene_pos_pos,
            rgb3d_feats_rot=rgb3d_feats_rot,
            fps_scene_feats_rot=fps_scene_feats_rot,
            fps_scene_pos_rot=fps_scene_pos_rot,
        )
    ...
    def compute_trajectory(self, trajectory_mask,
                           rgb3d, rgb2d, pcd, instruction, proprio, extra_inputs=None):
        # Encode observations, states, instructions
        fixed_inputs = self.encode_inputs(
            rgb3d, rgb2d, pcd, instruction, proprio, extra_inputs=extra_inputs
        )
...
class TransformerHead(nn.Module):
    ...
    def forward(self, traj_feats, trajectory, timesteps,
        ...
        rel_traj_pos_pos, rel_scene_pos_pos, rel_pos_pos = self.get_positional_embeddings(
            traj_xyz, traj_feats,
            rgb3d_pos, rgb3d_feats_pos, rgb2d_feats, rgb2d_pos,
            timesteps, proprio_feats,
            fps_scene_feats_pos, fps_scene_pos_pos,
            instr_feats, instr_pos
        )
        rel_traj_pos_rot, rel_scene_pos_rot, rel_pos_rot = self.get_positional_embeddings(
            traj_xyz, traj_feats,
            rgb3d_pos, rgb3d_feats_rot, rgb2d_feats, rgb2d_pos,
            timesteps, proprio_feats,
            fps_scene_feats_rot, fps_scene_pos_rot,
            instr_feats, instr_pos
        )

        # Cross attention from gripper to full context
        traj_feats_pos = self.cross_attn(
            seq1=traj_feats,
            seq2=rgb3d_feats_pos,
            seq1_pos=rel_traj_pos_pos,
            seq2_pos=rel_scene_pos_pos,
            ada_sgnl=time_embs
        )[-1]
        traj_feats_rot = self.cross_attn(
            seq1=traj_feats,
            seq2=rgb3d_feats_rot,
            seq1_pos=rel_traj_pos_rot,
            seq2_pos=rel_scene_pos_rot,
            ada_sgnl=time_embs
        )[-1]

        # Self attention among gripper and sampled context
        features_pos = self.get_sa_feature_sequence(
            traj_feats_pos, fps_scene_feats_pos,
            rgb3d_feats_pos, rgb2d_feats, instr_feats
        )
        features_pos = self.self_attn(
            seq1=features_pos,
            seq2=features_pos,
            seq1_pos=rel_pos_pos,
            seq2_pos=rel_pos_pos,
            ada_sgnl=time_embs
        )[-1]
        features_rot = self.get_sa_feature_sequence(
            traj_feats_rot, fps_scene_feats_rot,
            rgb3d_feats_rot, rgb2d_feats, instr_feats
        )
        features_rot = self.self_attn(
            seq1=features_rot,
            seq2=features_rot,
            seq1_pos=rel_pos_rot,
            seq2_pos=rel_pos_rot,
            ada_sgnl=time_embs
        )[-1]

        # Rotation head
        rotation = self.predict_rot(
            features_rot, rel_pos_rot, time_embs, traj_feats.shape[1]
        )

        # Position head
        position, position_features = self.predict_pos(
            features_pos, rel_pos_pos, time_embs, traj_feats.shape[1]
        )
